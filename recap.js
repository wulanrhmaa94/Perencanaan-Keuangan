  ...
  if (wishlist.length > 0) {
    wishlist.forEach((item, index) => {
      ...
      const recommendation = netBalance >= item.price
        ? `Boleh beli☺️ Sisa: Rp ${(netBalance - item.price).toLocaleString()}`
        : `Jangan dulu!, belum cukup😡🙅‍♀️`;
...
<img 
  src="${
    netBalance >= item.price
      ? 'https://media.giphy.com/media/111ebonMs90YLu/giphy.gif'  // GIF senang
      : 'https://media.tenor.com/vzaHBuySOH0AAAAj/cony-and.gif'    // GIF sedih
  }" 
  alt="${netBalance >= item.price ? 'boleh beli' : 'belum cukup'}" 
